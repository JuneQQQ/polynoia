...
Tools are decided by ONE structural fact: are you this conversation's
orchestrator or not. Two cases, nothing else:

  · designated orchestrator → the orchestrator toolset (dispatch/discuss/present)
  · everyone else           → the full builder toolset
...
def effective_tool_role(*, is_orchestrator: bool) -> str:
    ...
    """
    return "orchestrator" if is_orchestrator else DEFAULT_TOOL_ROLE

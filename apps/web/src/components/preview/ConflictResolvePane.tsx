...
type Choice = { mode: "ours" | "theirs" | "edit" | "keep" | "delete"; text: string };
...
export function ConflictResolvePane({ convId }: { convId: string }) {
    ...
    return (
      <div className="h-full grid place-items-center text-[12.5px] text-[var(--color-fg-3)] bg-[var(--color-surface-2)]">
        <div className="text-center px-6">
          ...
          <div className="text-[11px]">多个 Agent 改了同一处代码、合并 main 失败时,这里显示冲突并让你解决。</div>
        </div>
      </div>
    );
  }
  ...
  const baseLabel = baseAgents.length ? baseAgents.map(nameOf).join("、") : "主线"; // e.g. 码甲
  const agent = agents.find((a) => a.id === conflict.agent_id);

  const key = (p: string) => `${conflict.id}:${p}`;
  const choiceOf = (f: ConflictFile): Choice => over[key(f.path)] ?? defaultChoice(f);
  ...
  const resolve = async () => {
    ...
    for (const f of files) {
      ...
      else if (ch.mode === "keep") sides[f.path] = f.ours != null ? "ours" : "theirs";
    }
    try {
  ...
  return (
    <div className="h-full flex flex-col bg-[var(--color-surface)]">
      <div className="border-b border-[var(--color-line)] px-3 py-2 bg-[var(--color-surface-2)] flex items-center gap-2">
        <GitMerge size={13} style={{ color: "var(--color-amber)" }} className="flex-shrink-0" />
      ...
      <div className="flex-1 overflow-y-auto">
        {files.map((f) => {
          ...
          const fdiff = isText
            ? lineDiffUnified(f.ours ?? "", f.theirs ?? "", f.path, { context: 3 })
            : null;
          return (
            <div key={f.path} className="border-b border-[var(--color-line)]">
              ...
              <div className="px-3 py-2 bg-[var(--color-surface-2)] flex flex-wrap items-center gap-1.5">
                ...
                {isModDel ? (
                  <>
                    <ChoiceBtn on={ch.mode === "keep"} onClick={() => setChoice(f, { mode: "keep" })}>
                      保留文件
                    </ChoiceBtn>
                    <ChoiceBtn on={ch.mode === "delete"} onClick={() => setChoice(f, { mode: "delete" })}>
                      删除文件
                    </ChoiceBtn>
                  </>
                ) : (
                  <>
                    <ChoiceBtn on={ch.mode === "ours"} onClick={() => setChoice(f, { mode: "ours" })}>
                      采用 {baseLabel}
                    </ChoiceBtn>
                    <ChoiceBtn on={ch.mode === "theirs"} onClick={() => setChoice(f, { mode: "theirs" })}>
                      采用 {agentLabel}
                    </ChoiceBtn>
                    {isText && (
                      <ChoiceBtn
              ...
              {isBinary ? (
                <div className="px-3 py-2 text-[11px] text-[var(--color-fg-3)]">
                  二进制文件 — 无法逐行比较,只能整体采用一侧({baseLabel} / {agentLabel})。
                </div>
              ) : ch.mode === "edit" ? (
                <textarea
                  value={ch.text}
                  onChange={(e) => setChoice(f, { mode: "edit", text: e.target.value })}
                  spellCheck={false}
                ...
                <>
                  ...
                  <div className="max-h-72 overflow-y-auto text-[12px]">
                    <DiffView
                      data={
                        {
                          oldFile: { fileName: f.path, fileLang: inferLang(f.path), content: f.ours ?? "" },
                          newFile: { fileName: f.path, fileLang: inferLang(f.path), content: f.theirs ?? "" },
                          hunks: [fdiff?.unified ?? ""],
                        } as never
                      }

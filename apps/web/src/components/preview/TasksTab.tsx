...
export function TasksTab({ payload }: { payload?: TasksPayload | null }) {
	...
	const lanes = useMemo(() => {
		...
		const result: {
			agentId: string;
			blocks: { x: number; w: number; label: string; state: string }[];
		}[] = [];
		let cursor = 0;
	...
	const events: EventItem[] = useMemo(() => {
		if (!payload) return [];
		// Precompute labels here — the forEach below shadows `t` with the task var.
		const completedLabel = t("completed", lang);
		const inProgressLabel = t("inProgress", lang);
		const failedLabel = t("processStatusFailed", lang);
		const evts: EventItem[] = [
			{
				...
				text: (
					<>
						<b>Orchestrator</b> {t("splitTasks", lang)}{" "}
						<span className="text-[var(--color-fg-3)]">
							{payload.tasks.length} {t("parallelSubtasks", lang)}
						</span>
					</>
				),
			},
			...
			if (t.state === "done") {
				...
				evts.push({
					...
					text: (
						<>
							<b>{a?.name ?? t.agent}</b> {t.label}{" "}
							<span className="text-[var(--color-fg-3)]">· {failedLabel}</span>
						</>
					),
				});
			}
		...
		return (
			<div className="h-full grid place-items-center text-[12.5px] text-[var(--color-fg-3)] bg-[var(--color-surface-2)]">
				<div className="text-center">
					<CircleDot
						size={20}
						className="mx-auto mb-2 text-[var(--color-fg-4)]"
					/>
					<div className="mb-1">{t("noTaskOrchestration", lang)}</div>
					<div className="text-[11px]">
						{t("noTaskOrchestrationHint", lang)}
					</div>
				</div>
			</div>
		);
	...
	return (
		<div className="h-full overflow-y-auto p-4 bg-[var(--color-surface)]">
			...
			<div className="flex flex-wrap gap-4 mb-4 text-[11.5px] text-[var(--color-fg-3)]">
				<span>
					{t("statTime", lang)}{" "}
					<b className="text-[var(--color-fg)] mono">{totalTime}</b>
				</span>
				<span>
					{t("statCalls", lang)}{" "}
					<b className="text-[var(--color-fg)] mono">{totalCalls}</b>{" "}
					{t("times", lang)}
				</span>
				<span>
					{t("statTokens", lang)}{" "}
					<b className="text-[var(--color-fg)] mono">{totalTokens}</b> token
				</span>
				<span>
					{t("statCost", lang)}{" "}
					<b className="text-[var(--color-fg)] mono">{totalCost}</b>
				</span>
			</div>
				...
				<div className="bg-[var(--color-surface-2)] divide-y divide-[var(--color-line)]">
					{lanes.map((lane) => {
						...
						return (
							<div
								key={lane.agentId}
								className="grid grid-cols-[120px_1fr] items-center py-2"
							>
			...
			<div className="space-y-1 mb-4">
				{events.map((e, i) => (
					<div
						key={i}
						className="grid grid-cols-[44px_18px_1fr] items-center gap-2 text-[11.5px]"
					>

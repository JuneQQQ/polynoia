...
export function MarpPreview({ content, fileName }: { content: string; fileName?: string }) {
  ...
  const exportPptx = useCallback(async () => {
    ...
    for (const s of marpToSlides(content)) {
      ...
      if (s.title) {
        slide.addText(s.title, { x: 0.5, y: 0.3, w: 9, h: 0.9, fontSize: 28, bold: true });
      }
      if (s.body) {
        slide.addText(s.body, { x: 0.5, y: 1.4, w: 9, h: 3.8, fontSize: 16, valign: "top" });
      }
    }
  ...
  return (
    <div className="h-full flex flex-col bg-[#f2f2f2]">
      <div className="flex items-center gap-2 px-3 py-1.5 border-b border-[var(--color-line)] bg-[var(--color-surface-2)] text-[11px] flex-shrink-0">
        <span className="font-mono truncate flex-1 text-[var(--color-fg-2)]">{fileName ?? "幻灯"}</span>
        <button
          type="button"
          onClick={() => !error && printAsPdf(html, base, `<style>${css}</style>`)}
          disabled={!!error}

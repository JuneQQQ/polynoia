...
type Op = { tag: " " | "-" | "+"; text: string; oldLine: number; newLine: number };
...
function singleHunk(file: string, a: string[], b: string[], ops: Op[]): string[] {
  const n = a.length;
  const m = b.length;
  return [

    ...
    return (
      <div className="h-full grid place-items-center text-[12.5px] text-[var(--color-fg-3)] bg-[var(--color-surface-2)]">
        <div className="text-center px-6">
          ...
          <div className="text-[11px]">Agent 在手动模式下提交文件改动时,这里会显示绿色增量 + 接受/拒绝</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col bg-[var(--color-surface)]">
      ...
      <div className="border-b border-[var(--color-line)] px-3 py-2 bg-[var(--color-surface-2)] flex items-center gap-2">
        ...
        {diff.dels > 0 && (
          <span className="text-[10.5px] px-1.5 py-0.5 rounded font-mono"
            style={{ background: "var(--color-red-soft)", color: "var(--color-red)" }}>−{diff.dels}</span>
        )}
      </div>
      ...
      <div className="border-t border-[var(--color-line)] px-3 py-2.5 bg-[var(--color-surface-2)] flex items-center gap-2">
        {agent && (
          <span className="inline-flex items-center gap-1.5 text-[11px] text-[var(--color-fg-2)] mr-auto">
            <span className="w-4 h-4 rounded-full grid place-items-center text-white text-[8px] font-medium"
        ...
        >
          {busy === "reject" ? <Loader2 size={12} className="animate-spin" /> : <X size={12} />} 拒绝
        </button>
        <button
          type="button"
          disabled={busy !== null}
        ...
        >
          {busy === "accept" ? <Loader2 size={12} className="animate-spin" /> : <Check size={12} />} 接受
        </button>
      </div>
    </div>
  );

...
import { csvToXlsxBlob, downloadBlob, downloadText, printAsPdf } from "./exportUtils";
...
export function SheetPreview({ content, fileName }: { content: string; fileName: string }) {
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      <div className="flex items-center gap-2 px-3 py-1.5 border-b border-[var(--color-line)] bg-[var(--color-surface-2)] text-[11px] flex-shrink-0">
        <span className="font-mono truncate flex-1 text-[var(--color-fg-2)]">{fileName}</span>
        <ExportBtn label=".xlsx" onClick={() => downloadBlob(csvToXlsxBlob(content), `${base}.xlsx`)} />
        <ExportBtn label=".csv" onClick={() => downloadText(content, `${base}.csv`, "text/csv;charset=utf-8")} />
        <ExportBtn label="PDF" onClick={() => printAsPdf(rowsToHtml(rows), base, `<style>${TABLE_CSS}</style>`)} />
      </div>
      <div className="flex-1 overflow-auto p-3">
        {error ? (
          <pre className="text-[var(--color-red)] text-[12px] whitespace-pre-wrap">{error}</pre>
        ) : rows.length === 0 ? (
          ...
          <table className="border-collapse text-[12.5px]">
            ...
            <tbody>
              {body.map((r, ri) => (
                <tr key={ri}>
                  {head.map((_, ci) => (
                    <td key={ci} className="border border-[var(--color-line)] px-2 py-1">
                      {String(r[ci] ?? "")}
                    </td>
                  ))}
                </tr>

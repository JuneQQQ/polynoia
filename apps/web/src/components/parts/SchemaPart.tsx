  ...
  return (
    <div className="border border-[var(--color-line)] rounded-lg overflow-hidden bg-[var(--color-surface)] max-w-[640px]">
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[var(--color-line)] bg-[var(--color-surface-2)]">
        ...
        <span className="text-xs font-medium mono truncate flex-1">{payload.table}</span>
        ...
        <table className="w-full text-[11.5px]">
          <thead>
            <tr className="text-[10px] uppercase tracking-wider text-[var(--color-fg-3)] border-b border-[var(--color-line)]">
              <th className="text-left px-3 py-1.5 font-semibold">字段</th>
              <th className="text-left px-3 py-1.5 font-semibold">类型</th>
              <th className="text-left px-3 py-1.5 font-semibold">可空</th>
              <th className="text-left px-3 py-1.5 font-semibold">键</th>
            </tr>
          </thead>
          <tbody>
            {payload.fields.map((f) => (
              <tr key={f.name} className="border-b border-[var(--color-line)]/60 last:border-0">
                <td className="px-3 py-1.5 mono font-medium">{f.name}</td>
                <td className="px-3 py-1.5 mono text-[var(--color-fg-3)]">{f.type}</td>
                <td className="px-3 py-1.5 text-[var(--color-fg-3)]">{f.null ? "—" : "✗"}</td>
                <td className="px-3 py-1.5">
                  {f.key && (
                    <span
                      className="text-[9.5px] font-bold px-1.5 py-0.5 rounded"
                      style={{
                        background: f.key === "PK" ? "var(--color-accent-soft)" : "var(--color-blue-soft)",
                        color: f.key === "PK" ? "var(--color-accent)" : "var(--color-blue)",
                      }}
                    >
                      {f.key}
                    </span>
            ...
            return (
              ...
              >
                <span
                  ...
                  style={{
                    background: idx.recommend ? "var(--color-accent)" : "var(--color-line)",

...
export function ConflictPart({ payload }: { payload: ConflictPayload }) {
	...
	// Is a turn actually live? The auto-fix fires once when the conflict surfaces;
	// if its session errors it never retries, leaving the card stuck on a spinner
	// that lies. Gate the「自动合并中…」spinner on real activity so a stalled round
	// reads as stalled (and the 手动解决 hatch is the obvious next move).
	const agentStatus = useStore((s) =>
		selectAgentStatuses(s, scope?.convId ?? ""),
	);
	const someoneWorking = [...agentStatus.values()].some(
		...
		>
			...
			<div className="px-3 py-2 text-[12px] text-[var(--color-fg-2)]">
				<div className="mb-1.5">
					{files.length} 个文件冲突,合并{" "}
					<span className="font-mono">{payload.into}</span> 失败:
				</div>
			...
			<div className="flex items-center gap-1.5 px-3 py-2 border-t border-[var(--color-line)] bg-[var(--color-surface-2)]">
				{status === "resolved" ? (
					...
					<>
						{someoneWorking ? (
							<span
								className="inline-flex items-center gap-1.5 text-[11px] mr-auto"
								style={{ color: "var(--color-amber)" }}
							>
								<Loader2 size={12} className="animate-spin flex-shrink-0" />
								<span>auto 模式 · 协调者自动合并中…</span>
							</span>
						) : (
							// No live turn → the auto round finished without resolving (failed /
							// stalled). Tell the truth instead of spinning forever.
							<span
								className="inline-flex items-center gap-1.5 text-[11px] mr-auto"
								style={{ color: "var(--color-fg-3)" }}
							>
								<AlertTriangle
									...
									style={{ color: "var(--color-amber)" }}
								/>
								<span>auto 自动合并未完成,请手动接管</span>
							</span>
						)}
						{/* Escape hatch: the auto round can fail/stall (e.g. the
                orchestrator's session errored) with no other way out. Let the
                user open the resolve pane to merge by hand or re-hand it to the
                model. */}
						<button
							type="button"
							onClick={() => {
							...
							className="inline-flex items-center gap-1 px-2 py-1 text-[11px] rounded border border-[var(--color-line)] text-[var(--color-fg-2)] hover:text-[var(--color-amber)] hover:border-[var(--color-amber)] transition flex-shrink-0"
						>
							<GitMerge size={11} /> 手动解决
						</button>
					</>
				) : (
					<>

...
function bodyText(payload: ReasoningPayload): string {
  return payload.body
    .map((b) => (typeof b.c === "string" ? b.c : b.c.map((s) => ("text" in s ? s.text : "")).join("")))
    .join("\n")
    .trim();
}
  ...
  useEffect(() => {
    ...
    if (prevStreaming.current && !isStreaming) {
      ...
      if (startRef.current != null && thoughtSecs == null) {
        ...
        setThoughtSecs(Math.max(1, Math.round((now - startRef.current) / 1000)));
      }
    }
        ...
        <span className="font-mono uppercase tracking-[0.14em]">
          {isStreaming
            ...
            : (payload.seconds ?? thoughtSecs) != null
              ? `思考 ${payload.seconds ?? thoughtSecs} 秒`
              : "思考过程"}
        </span>
      </button>

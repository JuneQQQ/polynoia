...
export function CreateHubView({ onOpenConv }: Props) {
	...
	// 对话式创建: free-text description → heuristic suggestion → prefilled create modal.
	const [describe, setDescribe] = useState("");
	const [suggesting, setSuggesting] = useState(false);
	const [prefill, setPrefill] = useState<Awaited<
		ReturnType<typeof api.suggestContact>
	> | null>(null);

	const generateFromDescribe = async () => {
		const d = describe.trim();
		if (!d || suggesting) return;
		setSuggesting(true);
		try {
			const pf = await api.suggestContact(d);
			setPrefill(pf);
			setEditing(null);
			setModal("custom");
		} catch {
			// fall back to a blank create modal
			setPrefill(null);
			setModal("custom");
		} finally {
			setSuggesting(false);
	...
	const visible = useMemo(() => {
		...
		return installable.filter((a) => {
			...
			if (q) {
				const hay =
					`${a.name} ${a.role ?? ""} ${a.tagline ?? ""} ${(a.caps ?? []).join(" ")} ${a.handle}`.toLowerCase();
		...
		<main className="flex-1 flex flex-col bg-[var(--color-bg)] overflow-hidden">
            ...
            群聊 / 项目 在 Sidebar 顶部 + 进入 workspace 后的 "+ 新建对话" 建 */}
				<section className="px-5 pt-5">
					...
					{/* 对话式创建 Agent (rule.md): describe → infer config → prefilled form */}
					<div className="mt-3 max-w-[640px] rounded-lg border border-[var(--color-line)] bg-[var(--color-surface)] p-3">
				...
				<section className="px-5 pt-6 pb-8">
					...
					<div className="mb-3 flex flex-col gap-2">
						<div className="relative">
							<Search
								size={12}
								className="absolute left-2 top-1/2 -translate-y-1/2 text-[var(--color-fg-4)]"
							/>
							<input
								type="search"
								value={query}
							...
							<div className="flex flex-wrap gap-1.5">
								...
								{visibleProviders.map((p) => (
									<button
										...
										key={p.id}
										onClick={() =>
											setActiveProvider(p.id === activeProvider ? null : p.id)
										}
										className={`text-[10.5px] px-2 py-0.5 rounded-full border transition ${
											activeProvider === p.id

    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-[var(--color-line)]">
          <Search size={16} className="text-[var(--color-fg-3)] flex-shrink-0" />
          <input
            ref={inputRef}
            type="search"
            value={query}
        ...
        <div className="max-h-[60vh] overflow-y-auto">
          ...
          {query.trim() && localHits.length === 0 && crossHits.length === 0 && !loadingCross && (
            <div className="px-6 py-12 text-center text-[12px] text-[var(--color-fg-3)]">
              没有匹配的结果
            </div>
          )}
          ...
          {localHits.length > 0 && (
            <section className="py-3">
              <div className="px-4 mb-2 text-[9.5px] font-mono uppercase tracking-[0.22em] text-[var(--color-fg-3)] font-medium">
                在本对话内 · {localHits.length}
              </div>
              <ul>
                {localHits.map((h) => {
                  const agent = agents.find((a) => a.id === h.senderId);
                  const name = h.senderId === "you" ? "我" : agent?.name ?? h.senderId;
                  return (
                    <li key={h.msgId}>
                      <button
                        type="button"
                      ...
                      >
                        ...
                        >
                          {agent?.initials ?? (h.senderId === "you" ? "我" : "?")}
                        </span>
          ...
          {(crossHits.length > 0 || loadingCross) && (
            <section className="py-3 border-t border-[var(--color-line)]">
              <div className="px-4 mb-2 text-[9.5px] font-mono uppercase tracking-[0.22em] text-[var(--color-fg-3)] font-medium flex items-center gap-2">
                ...
                其它对话 · {loadingCross ? "..." : crossHits.length}
                {loadingCross && <Loader2 size={10} className="animate-spin ml-1" />}
              </div>
              <ul>
                {crossHits.map((c) => (
                  <li key={c.id}>
                    ...
                    >
                      ...
                      <ArrowRight size={12} className="text-[var(--color-fg-4)] flex-shrink-0" />
                    </button>
                  </li>
                ))}
              </ul>

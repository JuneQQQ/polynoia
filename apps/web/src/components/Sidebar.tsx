					...
					{wsConvs.length === 0 && (
						...
						>
							...
							<div className="text-[11.5px] leading-relaxed text-[var(--color-sidebar-muted)] mb-2.5">
								项目刚创建,还没有对话。指定参与者(包括
								Orchestrator)后开始单聊或群聊。
							</div>
					...
					{contactsOpen && (
						<>
							...
							{filteredContacts.map((a, idx) => {
								const isAdapter = Object.prototype.hasOwnProperty.call(
									adapterReady,
									a.id,
								);
                          ...
                          mockup. Sized 32px to match the design's r=16 dots. */}
											...
											{isAdapter && (
												<span
													title={
														ready ? t("online", lang) : t("offlineStatus", lang)
													}
...
function ConvRow({
	...
	const handleArchive = async () => {
		...
		try {
			await (conv.archived
				? api.unarchiveConv(conv.id)
				: api.archiveConv(conv.id));
			...
			>
				...
				<div className="min-w-0 flex-1">
					...
					<div className="text-[10.5px] text-[var(--color-sidebar-muted)] mt-0.5 truncate">
						{kindLabel} · {conv.members.length} 成员
						{conv.unread > 0 && !active && ` · ${conv.unread} 未读`}
					</div>
				</div>
			</button>
			<button
